...
**Password portability note:** auto-login passwords are still DPAPI-encrypted by default. That protects them at rest on the current Windows account, but also means they will not decrypt if you copy the portable folder to another PC. A portable master-password option is planned for machine-independent protected passwords.
